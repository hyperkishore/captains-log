...
function LoginContent() {
  ...
  useEffect(() => {
    if (error === "unauthorized_domain") {
      // Could show a toast, but for now the UI handles it
    }
  }, [error]);
      ...
      <div className="w-full max-w-sm mx-auto p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 text-sm">
            Personal activity tracking with AI insights
          </p>
        </div>

        {error === "unauthorized_domain" && (
          <div className="mb-4 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm text-center">
            Access restricted to @hyperverge.co accounts
          </div>
        )}
        ...
        {showGoogle ? (
          ...
          <div className="text-center text-zinc-500 text-sm">
            Authentication not available in development mode.
          </div>
        )}
